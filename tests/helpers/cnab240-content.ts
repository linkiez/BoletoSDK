...
export const updateLineField = (
  ...
  setField(chars, value, start, end);
  return chars.join('');
};

export const createMinimalCnab240Content = (): string => {
  const lines: string[] = [];
  ...
  setField(segmentP, '109', 58, 58); // Portfolio
  setField(segmentP, 'NF-001', 63, 77); // Billing document number
  setField(segmentP, '28022026', 78, 85); // Due date
  setField(segmentP, '000000000150000', 86, 100); // Amount 1500.00
  setField(segmentP, '01', 107, 108); // Species code
  setField(segmentP, 'N', 109, 109); // Acceptance
  setField(segmentP, '21012026', 110, 117); // Issue date
  setField(segmentP, '09', 228, 229); // Currency
  ...
  setField(batchTrailer, '000004', 18, 23); // Total records
